...
#include "../Structure/WSDONObjectDict.h"

std::shared_ptr<structure::WSDONObject>
WSDONConverters::ToWSDONObject(std::shared_ptr<structure::IndexDocument> indexDocument) {
    auto object = structure::WSDONObjectDict();
    auto wsdonArray = std::make_shared<structure::WSDONObject::array_type>();
    if (indexDocument->indices.size() != 0) {
        wsdonArray->reserve(indexDocument->indices.size());
    }
    for (auto &index : indexDocument->indices) {
        auto map = structure::WSDONObjectDict();
        map["IndexColumnName"] = index.indexColumnName;
        map["IndexColumnValue"] = index.indexColumnValue;
        wsdonArray->push_back(map.object);
    }
    object["indices"] = wsdonArray;
...
WSDONConverters::ToWSDONObject(std::shared_ptr<structure::IndexSignatureDocument> indexDocument) {
    auto object = structure::WSDONObjectDict();
    object["TableName"] = indexDocument->tableName;

    auto arr = std::make_shared<structure::WSDONObject::array_type>();
    arr->resize(indexDocument->indexSignatures.size());
    std::fill(arr->begin(), arr->end(), std::make_shared<structure::WSDONObject>());
    for (int i = 0; i < indexDocument->indexSignatures.size(); ++i) {
        auto current = indexDocument->indexSignatures[i];
        auto currentMap = structure::WSDONObjectDict();

        currentMap["IndexColumnName"] = current.indexColumnName;
        currentMap["IndexColumnPath"] = current.indexColumnPath;

        (*arr)[i] = currentMap.object;
    }
    object["IndexSignatures"] = arr;
...
WSDONConverters::ToIndexDocument(std::shared_ptr<structure::WSDONObject> wsdonObject) {
    ...
    auto wsdonArrayObject = (*wsdonObject->getObject())["indices"];
